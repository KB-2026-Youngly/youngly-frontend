  ...
  <section class="survey-page">
    <div class="survey-shell">
      ...
      <form v-else @submit.prevent="submit">
        <section v-for="question in questions" :key="question.questionId" class="question-card"><h2>{{ question.questionNo }}. {{ question.questionText }}</h2><label v-for="choice in question.choices" :key="choice.choiceId"><input v-model="answers[question.questionNo]" :name="`question-${question.questionNo}`" type="radio" :value="choice.choiceId" required><span>{{ choice.choiceText }}</span></label></section>
